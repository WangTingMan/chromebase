...
bool SharedMemory::MapAt(off_t offset, size_t bytes) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (memory_)
      break;
    //ReleaseReservation();
    DLOG( ERROR ) << "Warning: Disabled ReleaseReservation();";
  }
  if (!memory_) {
  ...
  DLOG( ERROR ) << "Warning: Disabled SharedMemoryTracker::GetInstance()->IncrementMemoryUsage(*this);";
  return true;
}
...
bool SharedMemory::Unmap() {
  ...
  DLOG( ERROR ) << "Warning: Disabled SharedMemoryTracker::GetInstance()->DecrementMemoryUsage( *this );";
